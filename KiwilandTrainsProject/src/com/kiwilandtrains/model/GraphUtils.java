...
public class GraphUtils {
	...
	public static int calculateDistanceOfRoute(Path p) {
		...
		for(int i=0; i<cityList.size(); i++) {
			if(i != (cityList.size() -1)) {
				...
				// if edge found for this particular src & dest
				RouteEdge e = null;
				if((e = CityGraphManager.getInstance().getCityGraph().getEdgeBetween(src, dest)) != null) {
					totalLength += e.getDistance();
				} else {
	...
	/**
	 * 
	 * Desc: Method to get all paths to dest from pivot in a hash. 
	 * 
	 * @param dest
	 * @param pivot
	 * @param cityPathListMap
	 * @return
	 */
	public static HashSet<Path> getPathHashToVertexFromPivot(CityVertex dest, CityVertex pivot, Map<CityVertex, ArrayList<Path>> cityPathListMap) {
		
		// create a list of paths to dest vertex
		HashSet<Path> pathsToVertex = new HashSet<Path>();
